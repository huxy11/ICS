...
static inline void check_bound(IOMap *map, paddr_t addr) {
  Assert(map != NULL && addr <= map->high && addr >= map->low,"address (0x%08x) is out of bound {%s} [0x%08x, 0x%08x] at pc = 0x%08x, ebp = 0x%08x", addr, (map ? map->name : "???"), (map ? map->low : 0), (map ? map->high : 0), cpu.pc, cpu.ebp);
}
